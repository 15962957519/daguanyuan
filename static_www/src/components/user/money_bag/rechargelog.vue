<template>
    <div>
        ...
        <yd-navbar bgcolor="#af773e" :fixed="true" >
            ...
            <p style="font-size: .3rem;color: #fff;" slot="center">充值记录</p>
            ...
            <div class="mui-content balance_record_container" style="padding-top: 1rem;">
                <div class="bottom" id="log">
                    <div id="item1" class="mui-control-content mui-active">
                        <!-- ================================================= -->
                        <div class="list" v-for="(item,key) in rechargeLog" >
                            <div class="top">
                                <div>充值</div>
                                <div>￥{{ item.account }}</div>
                            </div>
                            <div class="bottom">
                                <div>{{ item.ctime |time}}</div>
  ...
  export  default{
    ...
    methods:{
        ...
        category () {
            ...
            var url = "/rechargeLog?token=" + token;
            _that.$axios.get(url).then(function(response){

                _that.rechargeLog = response.data.rechargeLog;
