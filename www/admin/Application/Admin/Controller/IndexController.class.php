...
class IndexController extends BaseController {
    ...
    public function welcome(){
    	...
    	$count['goods'] =  M('goods')->where("1=1")->count();//商品区总数
        ...
        $result=M('admin_log')->alias('L')->join('__ADMIN__ A on L.admin_id=A.admin_id')->field('L.*,A.user_name')->order('log_id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->assign('admin_log',$result);
    ...
    public function changeTableVal(){  
        ...
        if($field == 'baidu_collection' && $value == 1){   //百度推广
            self::baidu_collection($id_value);
        }elseif ($field == 'baidu_collection' && $value == 2){  //订阅号
