...
class OrderController extends BaseController {
    ...
    public function ajaxindex(){
        ...
        I('consignee') ? $condition['aaa.consignee'] = trim(I('consignee')) : false;
        if($begin && $end){
        	$condition['aaa.add_time'] = array('between',"$begin,$end");
        }
        I('order_sn') ? $condition['aaa.order_sn'] = trim(I('order_sn')) : false;
        I('order_status') != '' ? $condition['aaa.order_status'] = I('order_status') : false;
        I('pay_status') != '' ? $condition['aaa.pay_status'] = I('pay_status') : false;
        I('pay_code') != '' ? $condition['aaa.pay_code'] = I('pay_code') : false;
        I('shipping_status') != '' ? $condition['aaa.shipping_status'] = I('shipping_status') : false;
        I('user_id') ? $condition['user_id'] = trim(I('user_id')) : false;
        $sort_order = I('order_by','DESC').' '.I('sort');

        $count = M('order')->alias('aaa')->where($condition)->fetchsql(true)->count();
        $condition['u.fictitious']  =0;
       	...
       	if(IS_POST)
        {
            ...
            if($goods_id_arr){
            	...
            	foreach($new_goods as $key => $val)
            	{
            		...
            		$rec_id = M('order_goods')->add($val);//订单添加商品区
            		if(!$rec_id)
            			$this->error('添加失败');
            	}
            }
            ...
            foreach ($orderGoods as $val){
            	if(empty($old_goods[$val['rec_id']])){
            		M('order_goods')->where("rec_id=".$val['rec_id'])->delete();//删除商品区
            	}else{
            		//修改商品区数量
            ...
            //################################修改订单费用
            $order['goods_price']    = $result['result']['goods_price']; // 商品区总价
    ...
    public function split_order(){
    	...
    	if(IS_POST){
    		...
    		foreach ($orderGoods as $val){
    			...
    			$all_goods[$val['rec_id']] = $val;//所有商品区信息
    		}
    		...
    		//修改订单费用
    		$res['goods_price']    = $result['result']['goods_price']; // 商品区总价
    		...
    		foreach($brr as $goods){
    			...
    			//修改订单费用
    			$new_order['goods_price']    = $result['result']['goods_price']; // 商品区总价
    			...
    			foreach ($goods as $vv){
    				...
    				unset($vv['rec_id']);
    				$nid = M('order_goods')->add($vv);//插入订单商品区表
    			}
    		}
    		//################################新单处理结束
    ...
    public function return_info()
    {
        ...
        if(IS_POST)
        {
            ...
            {        
            	...
            	M('order_goods')->where($where)->save(array('is_send'=>$type));//更改商品区状态        
                $orderLogic = new OrderLogic();
   ...
   {                
    ...
    public function export_order()
    {
    	...
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">商品区信息</td>';
    	$strTable .= '</tr>';
	    if(is_array($orderList)){
	    	...
	    	foreach($orderList as $k=>$val){
	    		...
	    		foreach($orderGoods as $goods){
	    			$strGoods .= "商品区编号：".$goods['goods_sn']." 商品区名称：".$goods['goods_name'];
        ...
        if(IS_POST)
        {
           ...
           $order['total_amount']   = $result['result']['total_amount']; // 订单总价
            ...
            if($order_id)
            {
                ...
                $this->success('添加商品区成功',U("Admin/Order/detail",array('order_id'=>$order_id)));
                exit();
            }
            else{
                $this->error('添加失败');
