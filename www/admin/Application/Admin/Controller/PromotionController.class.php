...
class PromotionController extends BaseController {
	...
	public function prom_goods_save()
	{
		...
		if($prom_id){
			...
			$last_id = $prom_id;
			adminLog("管理员修改了商品区促销 ".I('name'));
		}else{
		    unset($data['id']);
			$last_id = M('prom_goods')->add($data);
			adminLog("管理员添加了商品区促销 ".I('name'));
		}
		...
		if(is_array($data['goods_id'])){
			...
			if($prom_id>0){
				...
				//M('goods')->where($where)->save(array('prom_id'=>0,'prom_type'=>0));
                $is_have_arr = M('goods')->where($where)->select();
                //取二维数组的id 一维数组
                $ids = array_column($is_have_arr, 'goods_id');

                //判断长度
                $lengoods = count($data['goods_id']);
                //原来数据库数据长度 二维数组
                $sea_goods_len = count($is_have_arr);
			}

			if ($lengoods > $sea_goods_len){
                $goods_id = array_diff($data['goods_id'],$ids);
                $save_where['goods_id'] = array('in',$goods_id);
                M('goods')->where($save_where)->save(array('prom_id'=>$last_id,'prom_type'=>3));
            }else{
                $goods_id = array_diff($ids,$data['goods_id']);
                $save_where['goods_id'] = array('in',$goods_id);
                M('goods')->where($save_where)->save(array('prom_id'=>0,'prom_type'=>0));
