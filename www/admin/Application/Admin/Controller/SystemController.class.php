...
class SystemController extends BaseController{
	...
	{          
		/*配置列表*/
		$group_list = array('shop_info'=>'网站信息','basic'=>'基本设置','sms'=>'短信设置','shopping'=>'购物流程设置','smtp'=>'邮件设置','water'=>'水印设置','distribut'=>'分销设置','caution'=>'保证金');
     ...
     */
      public function ClearGoodsHtml(){
            ...
            if(unlink("./Application/Runtime/Html/Home_Goods_goodsInfo_{$goods_id}.html"))
            {
                ...
                foreach ($html_arr as $key => $val)
                {            
                    strstr($val,"Home_Goods_ajax_consult_{$goods_id}") && unlink($val); // 商品区咨询缓存
                    strstr($val,"Home_Goods_ajaxComment_{$goods_id}") && unlink($val); // 商品区评论缓存
                }
     ...
     */        
     ...
     function right_list(){
     	$group = array('system'=>'系统设置','content'=>'内容管理','goods'=>'商品区中心','member'=>'会员中心',
