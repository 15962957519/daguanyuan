...
class GoodsController extends BaseController {
    ...
    public function delGoodsCategory(){
        ...
        $goods_count > 0 && $this->error('该分类下有商品区不得删除!',U('Admin/Goods/categoryList'));
        // 删除分类
    ...
    public function ajaxGoodsList(){
        ...
        $where = ' 1 = 1 and  G.delete_time is null '; // 搜索条件
        I('intro')    && $where = "$where and "."G.".I('intro')." = 1" ;
        I('baidu_collection') && $where = "$where and G.baidu_collection = ".I('baidu_collection') ;
        ...
        if($key_word)
        {
            $where = "$where and (G.goods_name like '%$key_word%' or G.goods_content like '%$key_word%')" ;
        }
        if($nickname)
        {
            $where = "$where and (U.nickname like '%$nickname%')" ;
        }
        if($cat_id > 0)
        {
        ...
        $count = $model
            ->alias('G')
            ->join('__USERS__ U on G.user_id = U.user_id')
            ->where($where)
            ->count();
    ...
    public function addEditGoods()
    {
        ...
        if (($_GET['is_ajax'] == 1) && IS_POST) {
            ...
            if (!$Goods->create(NULL, $type))// 根据表单提交的POST数据创建数据对象
            {
                ...
                $GoodsLogic->saveGoodsAttr($goods_id, $_POST['goods_type']); // 处理商品区 属性
        ...
        $this->assign('goodsInfo', $goodsInfo);  // 商品区详情
        $goodsImages = M("GoodsImages")->where('goods_id =' . I('GET.id', 0))->select();
        foreach($goodsImages as $k=>$v){
            $res_id = $v['image_url'];
            $goodsImages[$k]['image_url'] = (new AliyunOss(true))->getCurlofimgUsenoAuth(C('aliyun.bucket'), $res_id);
        }
        $this->assign('goodsImages', $goodsImages);  // 商品区相册
        $this->initEditor(); // 编辑器
        $this->display('_goods');
    } 
    ...
    /**
     * 动态获取商品区属性输入框 根据不同的数据返回不同的输入框类型
     */
    ...
    public function delGoods()
    {
        ...
        $Goods->where('goods_id ='.$goods_id)->save($data);//商品区表
        $Goods->delete();

        M("cart")->where('goods_id ='.$goods_id)->delete();  // 购物车
        M("comment")->where('goods_id ='.$goods_id)->delete();  //商品区评论
        M("goods_consult")->where('goods_id ='.$goods_id)->delete();  //商品区咨询
        M("goods_images")->where('goods_id ='.$goods_id)->delete();  //商品区相册
        M("spec_goods_price")->where('goods_id ='.$goods_id)->delete();  //商品区规格
        M("spec_image")->where('goods_id ='.$goods_id)->delete();  //商品区规格图片
        M("goods_attr")->where('goods_id ='.$goods_id)->delete();  //商品区属性     
        M("goods_collect")->where('goods_id ='.$goods_id)->delete();  //商品区收藏          
    ...
    public function delGoodsType()
    {
        ...
        $count > 0 && $this->error('该类型下有商品区规格不得删除!',U('Admin/Goods/goodsTypeList'));
        // 判断 商品区属性        
        $count = M("GoodsAttribute")->where("type_id = {$_GET['id']}")->count("1");   
        $count > 0 && $this->error('该类型下有商品区属性不得删除!',U('Admin/Goods/goodsTypeList'));        
        // 删除分类
    ...
    {         
        ...
        $count > 0 && $this->error('有商品区使用该属性,不得删除!',U('Admin/Goods/goodsAttributeList'));                        
        // 删除 属性
    ...
    {        
        ...
        if($goods_count)
        {
            $return_arr = array('status' => -1,'msg' => '此品牌有商品区在用不得删除!','data'  =>'',);   //$return_arr = array('status' => -1,'msg' => '删除失败','data'  =>'',);        
            $this->ajaxReturn(json_encode($return_arr));            
        }
