...
class GoodsModel extends Model {
    ...
    protected $_validate = array(
        array('goods_name','require','商品区名称必须填写！',1 ,'',3),         
        //array('cat_id','require','商品区分类必须填写！',1 ,'',3),        
        array('cat_id','0','商品区分类必须填写。',1,'notequal',3),
        array('goods_sn','','商品区货号重复！',2,'unique',1),        
    ...
    {            
         ...
         // 商品区规格价钱处理
        $specGoodsPrice = M("SpecGoodsPrice"); // 实例化 商品区规格 价格对象
        $specGoodsPrice->where('goods_id = '.$goods_id)->delete(); // 删除原有的价格规格对象         
         if($_POST['item'])
         {
             ...
             foreach($_POST['item'] as $k => $v)
             {
                   ...
                   $store_count = $v['store_count'] = trim($v['store_count']); // 记录商品区总库存
                   $v['sku'] = trim($v['sku']);
         ...
         refresh_stock($goods_id); // 刷新商品区库存
    }
