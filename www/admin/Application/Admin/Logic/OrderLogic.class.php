...
class OrderLogic extends RelationModel
{
     ...
     */
    public function getOrderList($condition,$order='',$start=0,$page_size=10){
       // $res = M('order')->where($condition)->limit("$start,$page_size")->order($order)->select();
        $res =  M('order')->alias('aaa')->join('__USERS__ as u  ON aaa.user_id = u.user_id')->where($condition)->limit("$start,$page_size")->order($order)->fetchsql(false)->field('aaa.*')->select();
    ...
    function order_pay_cancel($order_id)
    {
    	...
    	{
    		if(!empty($val['spec_key']))// 有选择规格的商品区
    		{   // 先到规格表里面增加数量 再重新刷新一个 这件商品区的总数量
    			M('SpecGoodsPrice')->where("goods_id = {$val['goods_id']} and `key` = '{$val['spec_key']}'")->setInc('store_count',$val['goods_num']);
    			refresh_stock($val['goods_id']);
    		}else{
    			M('Goods')->where("goods_id = {$val['goods_id']}")->setInc('store_count',$val['goods_num']); // 增加商品区总数量
    		}
    		M('Goods')->where("goods_id = {$val['goods_id']}")->setDec('sales_sum',$val['goods_num']); // 减少商品区销售量
    		//更新活动商品区购买量
    ...
    public function deliveryHandle($data){
		...
		foreach ($orderGoods as $k=>$v){
			...
			if($v['is_send'] == 0 && in_array($v['rec_id'],$selectgoods)){
				$res['is_send'] = 1;
				$res['delivery_id'] = $did;
				$r = M('order_goods')->where("rec_id=".$v['rec_id'])->save($res);//改变订单商品区发货状态
				$is_delivery++;
			}
		}
