...
class AliyunOss
{
    ...
    {
        /*Vendor('aliyuncs.oss-sdk-php.src.OSS.Core.OssException');
        Vendor('aliyuncs.oss-sdk-php.src.OSS.OssClient');*/
        vendor('aliyuncs.oss-sdk-php.autoload');
        $this->accessKeyId = C('aliyun.accessKeyId');
        $this->accessKeySecret = C('aliyun.accessKeySecret');
        if ($isCName === true) {
            $this->endpoint = C('aliyun.custom');
        } else {
            $this->endpoint = C('aliyun.endpoint');
        }
    ...
    public function getCurlofimg($bucketName, $object, $timeout = 3600)
    {
        ...
        $options = array(
            \OSS\OssClient::OSS_PROCESS => 'image/resize,m_fixed,h_700,w_700/sharpen,100/watermark,image_' . $waterurl . ',t_90,g_se,x_10,y_10',
        );
    ...
    public function nomatergetCurlofimg($bucketName, $object, $timeout = 3600)
    {
        ...
        $options = array(
            \OSS\OssClient::OSS_PROCESS => 'image/resize,m_fill,h_160,w_160/sharpen,100',
        );
        $signedUrl = $this->ossClient->signUrl($bucketName, $object, $timeout, 'GET', $options);
    ...
    public function getnomatergetCurlofimgverfity($bucketName, $object, $timeout = 3600)
    {
         ...
         *  生成一个带签名的可用于浏览器直接打开的url, URL的有效期是3600秒
         */
        $waterurl = C('aliyunwatertextweixin.yongtu');
        $waterurl = rtrim(strtr(base64_encode($waterurl), '+/', '-_'), '=');
        ...
        $options = array(
            \OSS\OssClient::OSS_PROCESS => 'image/resize,m_fixed,h_1200,w_1200/sharpen,100/watermark,type_' . $fontType . ',size_40,text_' . $waterurl . ',color_FFFFFF,size_30,rotate_315,t_90,g_center,x_10,y_10',
        );
        $signedUrl = $this->ossClient->signUrl($bucketName, $object, $timeout, "GET", $options);
