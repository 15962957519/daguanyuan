...
class CartController extends MobileBaseController {
    ...
    function addCart()
    {
        $goods_id = I("goods_id"); // 商品区id
        $goods_num = I("goods_num");// 商品区数量
        $goods_spec = I("goods_spec"); // 商品区规格                
        $goods_spec = json_decode($goods_spec,true); //app 端 json 形式传输过来
        $unique_id = I("unique_id"); // 唯一id  类似于 pc 端的session id
        $user_id = I("user_id",0); // 用户id        
        $result = $this->cartLogic->addCart($goods_id, $goods_num, $goods_spec,$unique_id,$user_id); // 将商品区加入购物车
    ...
    function ajaxAddCart()
    {
        ...
        $goods_num = I("goods_num");// 商品区数量
        $goods_spec = I("goods_spec"); // 商品区规格
        $result = $this->cartLogic->addCart($goods_id, $goods_num, $goods_spec,$this->session_id,$this->user_id); // 将商品区加入购物车
        exit(json_encode($result));
    }
    ...
    {
        $cart_form_data = $_POST["cart_form_data"]; // goods_num 购物车商品区数量
    ...
    public function cart2()
    {
        ...
        if($this->cartLogic->cart_count($this->user_id,1) == 0 )
            $this->error ('你的购物车没有选中商品区','Cart/cart');

        $result = $this->cartLogic->cartList($this->user, $this->session_id,1,1); // 获取购物车商品区
        ...
        $this->assign('cartList', $result['cartList']); // 购物车的商品区
        $this->assign('total_price', $result['total_price']); // 总计
        $this->display();
    }

    /**
     * ajax 获取订单商品区价格 或者提交 订单
     */
    public function cart3(){
        ...
        if($this->cartLogic->cart_count($this->user_id,1) == 0 ) exit(json_encode(array('status'=>-2,'msg'=>'你的购物车没有选中商品区','result'=>null))); // 返回结果状态
    ...
    */
    ...
    {
        $post_goods_num = I("goods_num"); // goods_num 购物车商品区数量
