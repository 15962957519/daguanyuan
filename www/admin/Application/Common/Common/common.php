...
/**
 *  商品区缩略图 给于标签调用 拿出商品区表的 original_img 原始图来裁切出来的
...
/**
 * 刷新商品区库存, 如果商品区有设置规格库存, 则商品区总库存 等于 所有规格库存相加
 * @param type $goods_id  商品区id
 */
function refresh_stock($goods_id){
    ...
    M("Goods")->where("goods_id = $goods_id")->save(array('store_count'=>$store_count)); // 更新商品区的总库存
}
...
function accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0){
    ...
    /* 更新用户信息 */
    $sql = "UPDATE __PREFIX__users SET user_money = user_money + $user_money, frozen_money = frozen_money + $user_money," .
...
{    
    ...
    $goods_arr = M('goods')->where("goods_id in(".  implode(',',$goods_id_arr).")")->getField('goods_id,weight,market_price,is_free_shipping'); // 商品区id 和重量对应的键值对
