    ...
    */ 
    public function goodsInfo(){
        ...
        if(empty($goods) || ($goods['is_on_sale'] == 0)){
        	$this->error('该商品区已经下架',U('Index/index'));
        }
        if($goods['brand_id']){
            $brnad = M('brand')->where("id =".$goods['brand_id'])->find();
            $goods['brand_name'] = $brnad['name'];
        }
        $goods_images_list = M('GoodsImages')->where("goods_id = $goods_id")->select(); // 商品区 图册        
        $goods_attribute = M('GoodsAttribute')->getField('attr_id,attr_name'); // 查询属性
        $goods_attr_list = M('GoodsAttr')->where("goods_id = $goods_id")->select(); // 查询商品区属性表                        
        ...
        $commentStatistics = $goodsLogic->commentStatistics($goods_id);// 获取某个商品区的评论统计
        ...
        $this->assign('goods_images_list',$goods_images_list);//商品区缩略图
    ...
    public function goodsList(){ 
        ...
        $goodsLogic = new \Home\Logic\GoodsLogic(); // 前台商品区操作逻辑类
        ...
        {
            $goods_id_1 = $goodsLogic->getGoodsIdByBrandPrice($brand_id,$price); // 根据 品牌 或者 价格范围 查找所有商品区id    
            $filter_goods_id = array_intersect($filter_goods_id,$goods_id_1); // 获取多个帅选条件的结果 的交集
        }
        if($spec)// 规格
        {
            $goods_id_2 = $goodsLogic->getGoodsIdBySpec($spec); // 根据 规格 查找当所有商品区id
            $filter_goods_id = array_intersect($filter_goods_id,$goods_id_2); // 获取多个帅选条件的结果 的交集
        }
        if($attr)// 属性
        {
            $goods_id_3 = $goodsLogic->getGoodsIdByAttr($attr); // 根据 规格 查找当所有商品区id
        ...
        $this->assign('filter_brand',$filter_brand);  // 列表页帅选属性 - 商品区品牌
    ...
    public function ajax_consult(){        
        ...
        $consult_type = I('consult_type','0'); // 0全部咨询  1 商品区咨询 2 支付咨询 3 配送 4 售后
        ...
        $this->assign('consultCount',$count);// 商品区咨询数量
        $this->assign('consultList',$list);// 商品区咨询
    ...
    public function ajaxComment(){        
        ...
        $this->assign('commentlist',$list);// 商品区评论
        $this->assign('replyList',$replyList); // 管理员回复
        $this->assign('page',$show);// 赋值分页输出        
        $this->display();        
    }    
    ...
    public function goodsConsult(){
        //  form表单提交
        C('TOKEN_ON',true);        
        $goods_id = I("goods_id",'0'); // 商品区id
        $consult_type = I("consult_type",'1'); // 商品区咨询类型
    ...
    public function integralMall()
    {
        ...
        $this->assign('goods_category', $goods_category);//商品区1级分类
