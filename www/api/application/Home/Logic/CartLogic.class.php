...
class CartLogic extends RelationModel
{
    ...
    {       
        ...
        $goods = M('Goods')->where("goods_id = $goods_id")->find(); // 找出这个商品区        
        $specGoodsPriceList = M('SpecGoodsPrice')->where("goods_id = $goods_id")->getField("key,key_name,price,store_count,sku"); // 获取商品区对应的规格价钱 库存 条码

		$where = " session_id = '$session_id' ";
        $user_id = $user_id ? $user_id : 0;
		if($user_id)
		 $where .= "  or user_id= $user_id ";
        $catr_count = M('Cart')->where($where)->count(); // 查找购物车商品区总数量
        if($catr_count >= 20) 
            return array('status'=>-9,'msg'=>'购物车最多只能放20种商品区','result'=>'');            
        
        if(!empty($specGoodsPriceList) && empty($goods_spec)) // 有商品区规格 但是前台没有传递过来
            return array('status'=>-1,'msg'=>'必须传递商品区规格','result'=>'');                        
        if($goods_num <= 0) 
            return array('status'=>-2,'msg'=>'购买商品区数量不能为0','result'=>'');            
        if(empty($goods))
            return array('status'=>-3,'msg'=>'购买商品区不存在','result'=>'');            
        if(($goods['store_count'] < $goods_num))
            return array('status'=>-4,'msg'=>'商品区库存不足','result'=>'');        
        ...
        foreach($goods_spec as $key => $val) // 处理商品区规格
            $spec_item[] = $val; // 所选择的规格项                            
        if(!empty($spec_item)) // 有选择商品区规格
        {
            sort($spec_item);
        ...
        $where = " goods_id = $goods_id and spec_key = '$spec_key' "; // 查询购物车是否已经存在这商品区
        if($user_id > 0)
            $where .= " and (session_id = '$session_id' or user_id = $user_id) ";
        else
            $where .= " and  session_id = '$session_id' ";
        
        $catr_goods = M('Cart')->where($where)->find(); // 查找购物车是否已经存在该商品区
        $price = $spec_price ? $spec_price : $goods['shop_price']; // 如果商品区规格没有指定价格则用商品区原始价格
        ...
        $data = array(                    
                    ...
                    'goods_id'        => $goods_id,   // 商品区id
                    'goods_sn'        => $goods['goods_sn'],   // 商品区货号
                    'goods_name'      => $goods['goods_name'],   // 商品区名称
                    ...
                    'sku'        => "{$specGoodsPriceList[$spec_key]['sku']}", // 商品区条形码                    
     ...
     * @param type $selected  是否被用户勾选中的 0 为全部 1为选中  一般没有查询不选中的商品区情况
    ...
    {                   
        ...
        $cartList = M('Cart')->where($where)->select();  // 获取购物车商品区 
        ...
        foreach ($cartList as $k=>$val){
        	...
        	$cartList[$k]['store_count']  = getGoodNum($val['goods_id'],$val['spec_key']); // 最多可购买的库存数量        	
                ...
                // 如果要求只计算购物车选中商品区的价格 和数量  并且  当前商品区没选择 则跳过
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   return $freight;  
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        $order_count = M('Order')->where("user_id= $user_id and order_sn like '".date('Ymd')."%'")->count(); // 查找购物车商品区总数量
        if($order_count >= 50) 
   ...
   {
       $group_buy = M('GroupBuy')->where("goods_id = $goods_id and ".time()." >= start_time and ".time()." <= end_time ")->find(); // 找出这个商品区                      
       if(empty($group_buy))       
            return 0;
   ...
   public function login_cart_handle($session_id,$user_id)
   {
	   if(empty($session_id) || empty($user_id))
        ...
        if(!empty($cart_id_arr))
        {
            ...
            M('cart')->delete($cart_id_str); // 删除购物车完全相同的商品区
        }
   }
}