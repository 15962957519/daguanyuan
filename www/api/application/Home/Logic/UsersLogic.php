...
class UsersLogic
{
    ...
    public function thirdLogin($data=array()){
        ...
        if(!$user){
			...
			{
				// 送券            
				Db::name('coupon_list')->add(array('cid'=>$val['id'],'type'=>$val['type'],'uid'=>$user[user_id],'send_time'=>time()));
				Db::name('Coupon')->where("id = {$val['id']}")->setInconfig('send_num'); // 优惠券领取数量加一            
			}
    ...
    public function reg($username,$password,$password2){
        ...
        if(!empty($coupon)){
        	foreach ($coupon as $key => $val)
        	{
        		Db::name('coupon_list')->add(array('cid'=>$val['id'],'type'=>$val['type'],'uid'=>$user_id,'send_time'=>time()));
        		Db::name('Coupon')->where("id = {$val['id']}")->setInconfig('send_num'); // 优惠券领取数量加一
        	}
        }
    ...
    public function get_info($user_id){
        if(!$user_id > 0){
            ...
            $user_info = Db::name('users')->where('user_id',$user_id)->field('pay_points,real_name,nickname,user_id,sex,user_money,address_id,last_login,mobile,nickname,is_lock,usersingnature,token,user_type,user_level,is_authentication,mobile_validated,reg_time,head_pic,is_give_freefans,delete_time,store_level')->find();
        if(!$user_info)
            return false;
        //处理微信名问题
        $user_info['nickname'] = filterwname($user_info['nickname']);
        $user_info['usersingnature'] = filterwname($user_info['usersingnature']);
        if($user_info['delete_time']>0){
            return array('status'=>0,'msg'=>'获取成功','result'=>null);
        }
 ...
 * 获取当前登录用户信息
 */
    public function get_infobykey($user_id,$name){
        if(!$user_id > 0){
           return 0;
        }
        $user_info = Db::name('users')->where('user_id',$user_id)->value($name);
        return $user_info;
    }
     ...
     * 获取当前登录用户信息
     */
    public function get_infoThird($user_id)
    {
        $Users = new  ThirdUsers();
        $thirddata =$Users::where('user_id',$user_id)->field('openid')->find();
        return  $thirddata;
    }
     /*
      * 获取当前登录用户信息全部信息
      */
     public function get_infoAll(int $user_id){

         $Users = new  ThirdUsers();
         $Users_l = new  Users();
         $thirddata =$Users::where('user_id',$user_id)->field('oauth,lastsynctime,openid,weixinnumber,head_pic')->find();
         $Users_l_data =$Users_l::where('user_id',$user_id)->field('reg_time,user_id')->field('reg_time')->find();
         $now = Carbon::now();
         if($now->isThursday() && $thirddata['oauth']==1 && (($thirddata['lastsynctime']!=0 && time()-$thirddata['lastsynctime']>86400) || $thirddata['lastsynctime']==0) ){
             //先同步用户信息
             $jsapiobj = new  Weixin();
             $token = $jsapiobj->getAccessToeknCode();
             $userinfoobj =  $jsapiobj->getUserinfo($token,$thirddata['openid']);
             ...
             if(!isset($userinfoobj['errmsg']) &&  (isset($userinfoobj['subscribe']) && 1==$userinfoobj['subscribe'])) {
                 ...
                 try{
                     ...
                     //更新用户头像和姓名
                     $Users->where(['user_id'=>$user_id,'openid'=>$thirddata['openid']])->update($data);
                     unset($data['lastsynctime']);
                     $Users_l->where('user_id',$user_id)->update($data);
                     // 提交事务
                     Db::commit();
                 } catch (\Exception $e) {
                     // 回滚事务
             ...
             $thirddata =$Users::where('user_id',$user_id)->find();
         }
         $user_info_tmp =$this->get_info($user_id);
         $user_info['result']=null;
        if(isset($user_info_tmp['status']) && $user_info_tmp['status']==1){
            $user_info['result']=$user_info_tmp['result'];
        }

         $user_info['result']['timelevel'] = ProductServer::memberLevel($_SERVER['REQUEST_TIME'],$Users_l_data['reg_time']);
         $user_info['result']['weixinnumber'] =$thirddata['weixinnumber'];
         $user_info['result']['head_pic'] =$thirddata['head_pic'];
         $user_info['result']['collect_count'] = Db::name('fans_collect')->alias('a')->join('third_users w','a.user_id=w.user_id  and a.fans_id='.$user_id)->whereNull('a.delete_time')->whereNull('w.delete_time')->count(); //获取收藏数量    关注的
         $user_info['result']['fans_count'] = Db::name('fans_collect')->alias('a')->join('third_users w','a.fans_id=w.user_id and a.user_id='.$user_id)->whereNull('a.delete_time')->whereNull('w.delete_time')->count(); //获取收藏数量    粉丝
         $user_info['result']['waitPay']     = Db::name('order')->where("user_id = $user_id ".config('WAITPAY'))->count(); //待付款数量
         $user_info['result']['waitSend']    = Db::name('order')->where("user_id = $user_id ".config('WAITSEND'))->count(); //待发货数量  卖出的
         $user_info['result']['waitReceive'] = Db::name('order')->where("user_id = $user_id ".config('WAITRECEIVE'))->count(); //待收货数量     买入的
         $user_info['result']['order_count'] =  $user_info['result']['waitPay'] +  $user_info['result']['waitSend'] +  $user_info['result']['waitReceive'];
         $user_info['result']['message_count'] = Db::name('user_message')->where('user_id',$user_id)->cache('userinfo.user_message')->count();//消息数量
         $user_info['result']['auction']   = Db::name('Goods')->where('user_id' ,$user_id)->where('endTime','>',$_SERVER['REQUEST_TIME'])->whereNull('delete_time')->count();
         return array('status'=>1,'msg'=>'获取成功','result'=>$user_info);
     }


    public function getFancusLists(int $user_id,bool $flag=true,int $page ,int $limit){
     ...
     * 获取当前登录用户信息全部信息
     */
    public function getFancusAll(int $user_id,bool $flag=false,int $page ,int $limit){
        $start =$limit*($page-1);
        $limitstr = 'limit '.$start.','.$limit;
        if($flag===true){
//            $subQuery = Db::name('fans_collect')
//                ->field('*')
//                ->where('fans_id',$user_id)
//                ->buildSql();
//
//            $subQuery2 = Db::name('fans_collect')
//                ->field('*')
//                ->where('user_id',$user_id)
//                ->buildSql();

       $data=   Db::query("select  t1.* ,t2.user_id  as t2fid,tu.head_pic,tu.nickname,tmu.usersingnature,tmu.reg_time,tu.openid from (select * from tp_fans_collect where fans_id={$user_id} and delete_time is NULL ) as t1 left join (SELECT * from tp_fans_collect where user_id ={$user_id} and delete_time is NULL) as t2 on  t1.user_id=t2.fans_id  inner join tp_third_users  as tu on tu.user_id=t1.user_id inner join tp_users as tmu on tmu.user_id=t1.user_id  {$limitstr}");


         //   $data =  Db::name('third_users')->alias('a')->join([$subQuery=>'t1'],'t1.user_id=a.user_id')->join([$subQuery2=>'t2'],'t1.user_id=t2.fans_id','left')->join('users u','u.user_id=a.user_id')->paginate($limit);
           // $data =   Db::name('third_users')->alias('a')->join('fans_collect w','a.user_id=w.user_id')->join('fans_collect fw','fw.user_id=w.user_id')->join('users u','u.user_id=w.user_id')->field('a.*,u.usersingnature')->where('fans_id',$user_id)->paginate($limit); //我关注的
...
//                ->where('fans_id',$user_id)
//                ->buildSql();
//            $data =  Db::name('third_users')->alias('a')->join([$subQuery=>'t1'],'t1.user_id=a.user_id')->join([$subQuery2=>'t2'],'t1.fans_id=t2.user_id','left')->join('users u','u.user_id=a.user_id')->buildSql();
            $data=   Db::query("select  t1.* ,t2.fans_id  as t2fid,tu.head_pic,tu.nickname,tmu.usersingnature,tmu.reg_time,tu.openid from (select * from tp_fans_collect where user_id={$user_id} and delete_time is NULL) as t1 left join (SELECT * from tp_fans_collect where fans_id ={$user_id} and delete_time is NULL ) as t2 on  t1.fans_id=t2.user_id  inner join tp_third_users  as tu on tu.user_id=t1.fans_id inner join tp_users as tmu on tmu.user_id=t1.fans_id  {$limitstr}");


        //    $data =   Db::name('third_users')->alias('a')->join('fans_collect w','a.user_id=w.fans_id')->field('a.*,u.usersingnature')->where('user_id',$user_id)->paginate($limit); //我的粉丝
        }
     ...
     * 获取当前登录用户信息全部信息
     */
    public function getFancusAllAndRealy(int $user_id,bool $flag=false,int $page ,int $limit){
        $start =$limit*($page-1);
        $limitstr = 'limit '.$start.','.$limit;
        if($flag===true){
//            $subQuery = Db::name('fans_collect')
...
//                ->buildSql();
            ...
            $data=   Db::query("select  t1.* ,t2.user_id  as t2fid,tu.head_pic,tu.nickname,tmu.usersingnature,tmu.reg_time,tu.openid from (select * from tp_fans_collect where fans_id={$user_id} and delete_time is NULL ) as t1 left join (SELECT * from tp_fans_collect where user_id ={$user_id} and delete_time is NULL) as t2 on  t1.user_id=t2.fans_id  inner join tp_third_users  as tu on tu.user_id=t1.user_id inner join tp_users as tmu on tmu.user_id=t1.user_id  where tmu.fictitious=0  {$limitstr}");
...
//                ->buildSql();
//            $data =  Db::name('third_users')->alias('a')->join([$subQuery=>'t1'],'t1.user_id=a.user_id')->join([$subQuery2=>'t2'],'t1.fans_id=t2.user_id','left')->join('users u','u.user_id=a.user_id')->buildSql();
            $data=   Db::query("select  t1.* ,t2.fans_id  as t2fid,tu.head_pic,tu.nickname,tmu.usersingnature,tmu.reg_time,tu.openid from (select * from tp_fans_collect where user_id={$user_id} and delete_time is NULL) as t1 left join (SELECT * from tp_fans_collect where fans_id ={$user_id} and delete_time is NULL ) as t2 on  t1.fans_id=t2.user_id  inner join tp_third_users  as tu on tu.user_id=t1.fans_id inner join tp_users as tmu on tmu.user_id=t1.fans_id  where tmu.fictitious=0  {$limitstr}");
     ...
     * 获取账户资金记录
     */
    public function get_account_log($user_id,$type=0,$limit=16){
        //查询条件
//        $type = I('get.type',0);
        if($type == 1){
            //收入
        ...
        $count = Db::name('account_log')->where($where ??'user_id = '.$user_id)->count();
        $logs = Db::name('account_log')->where($where ?? 'user_id = '.$user_id)->order('change_time desc')->paginate($limit);
        $return['status'] = 1;
        $return['msg'] = '';
        $return['result'] = $logs;
        return $return;
    ...
    public function get_comment($user_id,$status=2){
        if($status == 1){
            //已评论
            $count2 = Db::name('')->query("select count(*) as count from `__PREFIX__comment`  as c inner join __PREFIX__order_goods as g on c.goods_id = g.goods_id and c.order_id = g.order_id where c.user_id = $user_id");
        	...
        	$comment = Db::name()->query($countsql);
        ...
        $show = $page->show();
        $comment_list = Db::name()->query($sql);
        if($comment_list){
    ...
    public function add_comment($add){
        ...
        //检查订单是否已完成
        $order = Db::name('order')->where("order_id = {$add[order_id]} AND user_id = {$add[user_id]}")->find();
        if($order['order_status'] != 2)
            return array('status'=>-1,'msg'=>'该笔订单还未确认收货','result'=>'');

        //检查是否已评论过
        $goods = Db::name('comment')->where("order_id = {$add[order_id]} AND goods_id = {$add[goods_id]}")->find();
        if($goods)            
            return array('status'=>-1,'msg'=>'您已经评论过该商品区','result'=>'');        
        ...
        if($row)
        {
            ...
            Db::name('goods')->where(array('goods_id'=>$add[goods_id]))->setInconfig('comment_count',1); // 评论数加一  
            // 查看这个订单是否全部已经评论,如果全部评论了 修改整个订单评论状态            
            $comment_count   = Db::name('order_goods')->where("order_id ='{$add[order_id]}' and is_comment = 0")->count();
            if($comment_count == 0) // 如果所有的商品区都已经评价了 订单状态改成已评价
            {
                Db::name('order')->where("order_id ='{$add[order_id]}'")->save(array('order_status'=>4));
            }
    ...
    public function add_address($user_id,$address_id=0,$data){
        ...
        //如果设为默认地址
        $insert_id = Db::name()->getLastInsID();
        $map['user_id'] = $user_id;
    ...
    {
        //检查用户是否已经有自提点
        $user_pickup_address_id = Db::name('user_address')->where('user_id=' . $user_id . ' AND is_pickup=1')->getField('address_id');
        ...
        if (!empty($user_pickup_address_id)) {
            //更新自提点
            Db::name('user_address')->where(array('address_id' => $user_pickup_address_id))->save($post);
    ...
    public function sms_log($mobile,$code,$session_id){
        //判断是否存在验证码
        $data = Db::name('sms_log')->where(array('mobile'=>$mobile,'session_id'=>$session_id))->order('id DESC')->find();
        //获取时间配置
        ...
        $row = Db::name('sms_log')->add(array('mobile'=>$mobile,'code'=>$code,'add_time'=>time(),'session_id'=>$session_id));
        if(!$row)
    ...
    public function sms_code_verify($mobile,$code,$session_id){
        //判断是否存在验证码
        $data = Db::name('sms_log')->where(array('mobile'=>$mobile,'session_id'=>$session_id,'code'=>$code))->order('id DESC')->find();
        if(empty($data))
        ...
        Db::name('sms_log')->where(array('mobile'=>$mobile,'session_id'=>$session_id,'code'=>$code))->delete();
     ...
     * 获取用户消息
     */
    public function getMessage(int $limit ,int $user_id,int $type=1)
    {
        $data=[];
        if ($user_id>0) {
            $data =   Db::name('message')->where(array('type' => $type))->paginate($limit);
    ...
    public function updateUserSinature(array $request,int $userid):bool {
         $userobj = new UserServer();
       return  $userobj->updateUserinfo($userid,$request);
    }
        public function thirdUpdateUserSinature(array $request,int $userid):bool {
         $userobj = new UserServer();
       return  $userobj->thirdUpdateUserinfo($userid,$request);
    }

    /*
     * 获取已经发送短信数量
     */
    public function getCountSendMessage(int $user_id):int {
        if(!$user_id > 0)
            return 0;
        $today = Carbon::today();
        $unixttimetodaytime = strtotime($today);
        $data = Db::name('user_sendmessage')->where('user_id',$user_id)->where('addtime', '>', $unixttimetodaytime)->value('sendactivemessagecount');
        if(!$data)
            return 0;
        return $data;
    }
     ...
     * 获取当前登录用户信息
     */
    public function get_infobyfield(int $user_id, string  $field="*"){
        if($user_id > 0){
            $user_info = Db::name('third_users')->where('user_id',$user_id)->field($field)->cache(true,10)->find();
            return  $user_info;
        }
        return ;
    }
