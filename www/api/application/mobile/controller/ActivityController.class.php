...
class ActivityController extends MobileBaseController {
    ...
    */ 
    public function group(){
        ...
        $group_buy_info = M('GroupBuy')->where("goods_id = $goods_id and ".time()." >= start_time and ".time()." <= end_time ")->find(); // 找出这个商品区
        if(empty($group_buy_info)) 
        {
            //$this->error("此商品区没有团购活动",U('Home/Goods/goodsInfo',array('id'=>$goods_id)));
        }
    ...
    public function group_list()
    {
        ...
        $list = M('GroupBuy')->where(time()." >= start_time and ".time()." <= end_time ")->limit($Page->firstRow.','.$Page->listRows)->select(); // 找出这个商品区        
