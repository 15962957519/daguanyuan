...
class UserBidServer
{
   ...
   */
    public static function indexbid(array $data)
    {
        ...
        $countmax = Db::name('goods')->where('goods_id', $good_id)->field('is_toplatform,upload_time,goods_id,endTime,last_update')->find();

        if ($countmax['upload_time'] && $countmax['endTime'] > time()) {
                $goodmidfytime =  $countmax['upload_time'];
            if($countmax['last_update']>0){
                $goodmidfytime =$countmax['last_update'];
            }
            $countmax_id = Db::name('forindex')->where('goods_id', $good_id)->where('datetime', '>', $goodmidfytime)->where('is_over', 1)->count('id');
