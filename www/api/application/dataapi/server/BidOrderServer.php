...
class BidOrderServer
{
    ...
    public function listsarraysbyuserid(int $user_id, int $limit, $flag = false, array $config = ['page' => 1])
    {
        $lists = Db::name('bid_order')->alias('a')->join('tp_goods b','a.goods_id = b.goods_id')->where('a.user_id', $user_id)->whereNull('a.delete_time')->field('a.id,a.user_id,a.bid_price,a.goods_id,a.upload_time,a.add_time,b.goods_name,b.start_price,b.upload_time')->order('a.bid_price desc')->paginate($limit, $flag, $config);
        if (!$lists->isEmpty()) {
            $lists = $lists->toArray();
            if (isset($lists['data'])) {
                foreach ($lists['data'] as &$v) {
                    $v['add_time'] = date('y-m-d H:i', $v['add_time']);
                    $v['bid_price'] = number_format($v['bid_price'], 2);
                    $userinfo = getuserinfofromcache($v['user_id']);
                    if(!empty($userinfo)){
                        if (!empty($userinfo['nickname'])) {
                            $v['nickname'] = preg_replace('/([a-zA-Z\d_]{5,})/', '****', $userinfo['nickname']);
                        } else {
                            $v['nickname'] = '';
                        }
                        $v['head_pic']=$userinfo['head_pic'];
                    }else{
                        $v['nickname'] = '';
                        $v['head_pic']='';
                    }
...
// 启动事务
        ...
        try{
            $goods = new   Goods();
            $datagood = $goods->where('goods_id', $goods_id)->where('is_toplatform in (1,3)')->where(['goods_status' => 1])->field('start_price,every_add_price,upload_time,last_update,endTime,protect_price,reserveprice')->lock(true)->find();
            if (!empty($datagood)) {
                $datagood = $datagood->toArray();
            } else {
                Db::rollback();
            ...
            $goodmidfytime =  $datagood['upload_time'];
            if($datagood['last_update']>0){
                $goodmidfytime =$datagood['last_update'];
            }



            $bidorderdata = $bidorder->where(['goods_id' => $goods_id, 'upload_time' =>$goodmidfytime])->order('bid_price desc')->value('bid_price');
            $checkuser_id = $bidorder->where(['goods_id' => $goods_id, 'upload_time' =>$goodmidfytime])->order('add_time desc')->field('user_id')->find();
            ...
            if (empty($bidorderdata) || $bidorderdata == 0 || $bidorderdata < $datagood['start_price']) {
                ...
                if ($datagood['protect_price'] > $datagood['start_price'] && $bidorderdata > $datagood['protect_price']) {
                    Db::rollback();
                    return 0;
                }
                $bid_price = bcadd($bidorderdata, $datagood['every_add_price'], 2);
