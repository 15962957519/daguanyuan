...
class AliyunOss
{
    ...
    public  function __construct($isCName=true)
    {
        ...
        $this->cdntianbao = Config::get('aliyun.cdntianbao');
    ...
    public function getCurlofimg($bucketName,$object,$timeout=3600){
         ...
         */
        $waterurl ='logo/weipai.png?x-oss-process=image/resize,P_20';
     ...
     * @return  json
     */
    public function getCurlofimgUsenoAuth($bucketName,$object,$width=350,$height=350,$is_water=0){
        $waterurl ='logo/weipai.png?x-oss-process=image/resize,P_20';
        $waterurl = rtrim(strtr(base64_encode($waterurl), '+/', '-_'), '=');
        $waterurl_t ='image/resize,m_pad,h_'.$width.',w_'.$height.'/sharpen,100/watermark,image_'.$waterurl.',t_90,g_se,x_10,y_20,image/bright,10';

        if($is_water != 0){
            $waterurl_t ='image/resize,m_pad,h_'.$width.',w_'.$height.'/sharpen,100,t_90,g_se,x_10,y_35,color_808080,image/bright,10';
        }

        if(  $this->isCName){
            $thhpimgurl = $this->cdntianbao.'/'.$object.'?x-oss-process='.$waterurl_t;
     ...
     * @return  json
     */
    public function getCurlofimgUsenoAuthfmort($bucketName,$object,$width=350,$height=350,$fmort='m_fill'){
        $waterurl ='logo/weipai.png?x-oss-process=image/resize,P_20';
        $waterurl = rtrim(strtr(base64_encode($waterurl), '+/', '-_'), '=');
        $width = mt_rand(350,550);
        $waterurl_t ='image/resize,'.$fmort.',h_'.$width.',w_'.$height.'/sharpen,100/watermark,image_'.$waterurl.',t_90,g_se,x_10,y_20,image/bright,10';
