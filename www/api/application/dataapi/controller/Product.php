...
//获取用户下面所有的产品
    ...
    public function categorylist(Request $request){
            $ProductServerobj = new ProductServer();
            $limit =$request->param('pagesize')??4;
            $cat_id= $request->param('cat_id')??0;
            $page  = $request->param('page')>1?(int)$request->param('page'):1;
            $selectdata = $ProductServerobj->getProductsOfcategory($limit,(int)$cat_id,$page);
            Response::create(['status' => '200', 'code' => '1', 'message' => '产品刘表成功', 'plists'=>$selectdata], 'json')->header($this->header)->send();
    }
    ...
    public function getBondlists(Request $request){
        ...
        if($goods_id){
            ...
            Response::create(['status' => '200', 'code' => '1', 'message' => '议价列表成功1', 'data'=>['surplus'=>$flag,'goods_id'=>(string)$goods_id,'plists'=>$bidlists['data']??[]]], 'json')->header($this->header)->send();
        }else{
            $userid =  $this->user_data['user_id'];
            $ProductServerobj = new ProductServer();
            $config['page']= (int)$request->param('page');
            $bidlists =   $ProductServerobj->getBondlistsbyuserid($userid,4,false,$config);
            //判断是否到最后一页了
            $flag='0';
            if(isset($bidlists['total'])){
                $totalpage =  ceil($bidlists['total']/$bidlists['per_page']);
            ...
            Response::create(['status' => '200', 'code' => '1', 'message' => '议价列表成功2', 'data'=>['surplus'=>$flag,'goods_id'=>(string)$goods_id,'plists'=>$bidlists['data']??[]]], 'json')->header($this->header)->send();
        }
        Response::create(['status' => '4000', 'code' => '1', 'message' => '议价列表成功'], 'json')->header($this->header)->send();
    }
    ...
    public function indexpaimaiprolist(Request $request){
        ...
        $user_info = $userLogic->get_info($user_id);
        $selectdata = $ProductServerobj->getProductsForpaimaiquan($limit,$user_id,$type,$self_user_id ,$goods_id,$page);
        Response::create(['status' => '200', 'code' => '1', 'user'=>$user_info['result'],'message' => '产品刘表成功', 'plists'=>$selectdata], 'json')->header($this->header)->send();
    }
    ...
    public function addclick(Request $request){
        ...
        $goods_id= $request->param('goods_id')??0;
        if($goods_id>0){
            $ProductServerobj->addclick($goods_id);
            Response::create(['status' => '200', 'code' => '1', 'message' => '浏览成功'], 'json')->header($this->header)->send();
        }else{
            Response::create(['status' => '200', 'code' => '1', 'message' => '浏览失败'], 'json')->header($this->header)->send();
        }
    }
