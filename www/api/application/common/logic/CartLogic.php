...
{
    protected $goods;//商品区模型
    protected $specGoodsPrice;//商品区规格模型
    protected $goodsBuyNum;//购买的商品区数量
    protected $session_id;//session_id
    protected $user_id = 0;//user_id
    protected $userGoodsTypeCount = 0;//用户购物车的全部商品区种类
    ...
    public function buyNow(){
        if(empty($this->goods)){
            return ['status'=>0,'msg'=>'购买商品区不存在','result'=>''];
        }
        $buyGoods = [
            ...
            'prom_id' => 0,   // 活动id
            'weight' => $this->goods['weight'],   // 商品区重量
            'goods'=>$this->goods,
        ];
        if(empty($this->specGoodsPrice)){
            ...
            if($specGoodsPriceCount > 0){
                return ['status' => 0, 'msg' => '必须传递商品区规格', 'result' => ''];
            }
    ...
    {
        if(empty($this->goods)){
            return ['status'=>-3,'msg'=>'购买商品区不存在','result'=>''];
        }
        if($this->goods['exchange_integral'] > 0){
            return ['status'=>0,'msg'=>'积分商品区跳转','result'=>['url'=>U('Goods/goodsInfo',['id'=>$this->goods['goods_id'],'item_id'=>$this->specGoodsPrice['item_id']],'',true)]];
        }
        $userCartCount = Db::name('cart')->where(['user_id'=>$this->user_id,'session_id'=>$this->session_id])->count();//获取用户购物车的商品区有多少种
        if ($userCartCount >= 20) {
            return array('status' => -9, 'msg' => '购物车最多只能放20种商品区', 'result' => '');
        }
        $specGoodsPriceCount = Db::name('SpecGoodsPrice')->where("goods_id", $this->goods['goods_id'])->count('item_id');
        if(empty($this->specGoodsPrice) && !empty($specGoodsPriceCount)){
            return array('status' => -1, 'msg' => '必须传递商品区规格', 'result' => '');
        }
        //有商品区规格，和没有商品区规格
    ...
    private function addNormalCart(){
        ...
        if ($userCartGoods) {
            ...
            if($userWantGoodsNum > $store_count){
                return array('status' => -4, 'msg' => '商品区库存不足，剩余'.$store_count.',当前购物车已有'.$userCartGoods['goods_num'].'件', 'result' => '');
            }
            $cartResult = $userCartGoods->save(['goods_num' => $userWantGoodsNum,'goods_price'=>$price,'member_goods_price'=>$price]);
        }else{
            //如果该商品区没有存在购物车
            if($this->goodsBuyNum > $store_count){
                return array('status' => -4, 'msg' => '商品区库存不足，剩余'.$this->goods['store_count'], 'result' => '');
            }
            //如果有阶梯价格,就是用阶梯价格
            if (!empty($this->goods['price_ladder'])) {
                $goodsLogic = new GoodsLogic();
            ...
            $cartAddData = array(
                ...
                'goods_id' => $this->goods['goods_id'],   // 商品区id
                'goods_sn' => $this->goods['goods_sn'],   // 商品区货号
                'goods_name' => $this->goods['goods_name'],   // 商品区名称
            ...
            if($this->specGoodsPrice){
                ...
                $cartAddData['sku'] = $this->specGoodsPrice['sku']; //商品区条形码
            }
    ...
    private function addFlashSaleCart(){
        ...
        if(!$flashSaleIsAble){
            ...
            //活动进行中
            if ($this->user_id == 0) {
                return array('status' => -101, 'msg' => '购买活动商品区必须先登录', 'result' => '');
            }
        }
        ...
        $userCartGoodsNum = empty($userCartGoods) ? 0 : $userCartGoods['goods_num'];///获取用户购物车的抢购商品区数量
        $userFlashOrderGoodsNum = $flashSaleLogic->getUserFlashOrderGoodsNum($this->user_id); //获取用户抢购已购商品区数量
        ...
        if($userWantGoodsNum > $flashSalePurchase){
            return array('status' => -4, 'msg' => '商品区库存不足，剩余'.$flashSalePurchase.',当前购物车已有'.$userCartGoodsNum.'件', 'result' => '');
        }
        // 如果该商品区已经存在购物车
        if($userCartGoods){
            ...
            if($this->specGoodsPrice){
                ...
                $cartAddFlashSaleData['sku'] = $this->specGoodsPrice['sku']; //商品区条形码
    ...
    private function addGroupBuyCart(){
        ...
        $userCartGoodsNum = empty($userCartGoods) ? 0 : $userCartGoods['goods_num'];///获取用户购物车的团购商品区数量
        $userWantGoodsNum = $userCartGoodsNum + $this->goodsBuyNum;//购物车加上要加入购物车的商品区数量
        $groupBuyPurchase = $groupBuy['goods_num'] - $groupBuy['buy_num'];//团购剩余库存
        if($userWantGoodsNum > 200){
            $userWantGoodsNum = 200;
        }
        if($userWantGoodsNum > $groupBuyPurchase){
            return array('status' => -4, 'msg' => '商品区库存不足，剩余'.$groupBuyPurchase.',当前购物车已有'.$userCartGoodsNum.'件', 'result' => '');
        }
        // 如果该商品区已经存在购物车
        if($userCartGoods){
            $cartResult = $userCartGoods->save(['goods_num' => $userWantGoodsNum]);
    ...
    /**
     * @param int $selected|是否被用户勾选中的 0 为全部 1为选中  一般没有查询不选中的商品区情况
     * 获取用户的购物车列表
    ...
    public function getCartList($selected = 0){
        ...
        $cartList = $cart->with('promGoods,goods')->where($cartWhere)->select();  // 获取购物车商品区
        $cartCheckAfterList = $this->checkCartList($cartList);
//        $cartCheckAfterList = $cartList;
        $cartGoodsTotalNum = array_sum(array_map(function($val){return $val['goods_num'];}, $cartCheckAfterList));//购物车购买的商品区总数
    ...
    public function doUserLoginHandle()
    {
        ...
        //登录后将购物车的商品区的 user_id 改为当前登录的id
        $cart = new Cart();
        ...
        if (!empty($cart_id_arr)) {
            ...
            M('cart')->delete($cart_id_arr); // 删除购物车完全相同的商品区
        }
    }
    /**
     * 更改购物车的商品区数量
     * @param $cart_id|购物车id
     * @param $goods_num|商品区数量
     * @return array
     */
    public function changeNum($cart_id, $goods_num){
        ...
        if($goods_num > $cart->limit_num){
            return ['status' => 0, 'msg' => '商品区数量不能大于'.$cart->limit_num, 'result' => ['limit_num'=>$cart->limit_num]];
        }
        if($goods_num > 200){
            $goods_num = 200;
        }
        ...
        $cart->save();
        return ['status' => 1, 'msg' => '修改商品区数量成功', 'result' => ''];
    }
    /**
     * 删除购物车商品区
    ...
    public function AsyncUpdateCart($cart = [])
    {
        ...
        if (empty($cart)) {
            return ['status' => 0, 'msg' => '购物车没商品区', 'result' => compact('total_fee', 'goods_fee', 'goods_num', 'cartList')];
        }
        ...
        if (empty($cartSelectedId)) {
            ...
            return ['status' => 1, 'msg' => '购物车没选中商品区', 'result' => $cartPriceInfo];
        }
    ...
    public function getCartPriceInfo($cartList = null){
        $total_fee = $goods_fee = $goods_num = 0;//初始化数据。商品区总额/节约金额/商品区总共数量
        if($cartList){
    ...
    public function getCouponCartList($cartList, $userCouponList)
    {
        ...
        foreach ($userCouponArray as $couponKey => $couponItem) {
            if ($userCouponArray[$couponKey]['coupon']['use_type'] == 0) { //全店使用优惠券
                if ($cartList['total_fee'] >= $userCouponArray[$couponKey]['coupon']['condition']) {  //订单商品区总价是否符合优惠券购买价格
                    $userCouponArray[$couponKey]['coupon']['able'] = 1;
                } else {
                    $userCouponArray[$couponKey]['coupon']['able'] = 0;
                }
            } elseif ($userCouponArray[$couponKey]['coupon']['use_type'] == 1) { //指定商品区优惠券
                $pointGoodsPrice = 0;//指定商品区的购买总价
                ...
                foreach ($cartList['cartList'] as $tKey => $Item) {
                    if (in_array($Item['goods_id'], $couponGoodsId)) {
                        $pointGoodsPrice += $Item['goods_price'] * $Item['goods_num'];  //统计每个商品区的总价
                    }
                }
            ...
            } elseif ($userCouponArray[$couponKey]['coupon']['use_type'] == 2) { //指定商品区分类优惠券
                $pointGoodsCatPrice = 0;//指定商品区分类的购买总价
