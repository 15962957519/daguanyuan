...
class OrderLogic
{
	...
	public function addReturnGoods($rec_id,$order)
	{
		...
		if($data['type'] < 2){
			//退款申请，若该商品区有赠送积分或优惠券，在平台操作退款时需要追回
    ...
    public function recordRefundOrder($user_id, $order_id, $user_note, $consignee, $mobile)
    {
    	...
    	if($order_return_num > 0){
    		return ['status' => -1, 'msg' => '该订单中有商品区正在申请售后'];
    	}
    	$order_status = 3;//已取消
    	$order_info = [
    	'user_note' => $user_note,
	...
	public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price=[],$user_note='',$pay_name='')
	{
		...
		//$order_count = M('Order')->where("user_id",$user_id)->where('order_sn', 'like', date('Ymd')."%")->count(); // 查找购物车商品区总数量
	...
	public function addPreSellOrder($user_id,$address_id,$shipping_code,$invoice_title,$act_id,$pre_sell_price)
	{
		...
		$data = array(
				...
				'invoice_title'    =>$invoice_title, //'发票抬头',
				'goods_price'      =>$pre_sell_price['cut_price'] * $pre_sell_price['goods_num'],//'商品区价格',
...
//        M('goods_activity')->where(array('act_id'=>$act_id))->setInc('act_count',$pre_sell_price['goods_num']);
		...
		$data2['goods_id']           = $goods['goods_id']; // 商品区id
		$data2['goods_name']         = $goods['goods_name']; // 商品区名称
		$data2['goods_sn']           = $goods['goods_sn']; // 商品区货号
		$data2['goods_num']          = $pre_sell_price['goods_num']; // 购买数量
		$data2['market_price']       = $goods['market_price']; // 市场价
		$data2['goods_price']        = $goods['shop_price']; // 商品区团价
		$data2['cost_price']         = $goods['cost_price']; // 成本价
		$data2['member_goods_price'] = $pre_sell_price['cut_price']; //预售价钱
		$data2['give_integral']      = $goods_activity['integral']; // 购买商品区赠送积分
		$data2['prom_type']          = 4; // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠 ,4 预售商品区
	...
	/**
	 * 取消订单后改变库存，根据不同的规格，商品区活动修改对应的库存
	 * @param $order
     * @param $rec_id|订单商品区表id 如果有只返还订单某个商品区的库存,没有返还整个订单
     */
		...
		foreach($order_goods as $key=>$val){
			if(!empty($val['spec_key'])){ // 先到规格表里面扣除数量
				...
				if($specGoodsPrice){
					...
					$specGoodsPrice->save();//有规格则增加商品区对应规格的库存
				}
			}else{
				M('goods')->where(['goods_id' => $val['goods_id']])->setInc('store_count', $val['goods_num']);//没有规格则增加商品区库存
			}
			update_stock_log($order['user_id'], $val['goods_num'], $val, $order['order_sn']);//库存日志

			Db::name('Goods')->where("goods_id", $val['goods_id'])->setDec('sales_sum', $val['goods_num']); // 减少商品区销售量
			//更新活动商品区购买量
