...
class GoodsLogic extends Model
{
    ...
    public function collect_goods($user_id, $goods_id)
    {
        ...
        if ($count > 0) return array('status' => -3, 'msg' => '商品区已收藏', 'result' => array());
    ...
    function getGoodsIdBySpec($spec)
    {
        ...
        $result = Db::query($sql);
        $arr = get_arr_column($result, 'goods_id');  // 只获取商品区id 那一列
    ...
    /**
     * 在有价格阶梯的情况下，根据商品区数量，获取商品区价格
     * @param $goods_num|购买的商品区数
     * @param $goods_price|商品区默认单价
...
//        $goods_where['goods_name'] = array("exp", " NOT REGEXP '华为|荣耀|小米|合约机|三星|魅族' ");//临时屏蔽,苹果APP审核过了之后注释
    ...
    public function integralMall($rank, $user_id, $p = 1)
    {
        ...
        $goods_where['is_virtual'] = 0;//是否虚拟商品区
        //积分兑换筛选
    ...
    public function getGoodsPromSimpleInfo($goods, $goodsPromLogic)
    {
        ...
        if (!$goodsPromFactory->checkPromType($goods['prom_type'])
            ...
            if ($prom['type'] == 0) {
                $activityData[] = ['title' => '折扣', 'content' => "指定商品区立打{$prom['expression']}折"];
            } elseif ($prom['type'] == 1) {
                $activityData[] = ['title' => '直减', 'content' => "指定商品区立减{$prom['expression']}元"];
            } elseif ($prom['type'] == 2) {
