        ...
        if ($goods_id > 0) {
            ...
            if ($user_id > 0) {
                ...
                if ($self_user_id > 0) {
                    ...
                    $order = ' order by g.on_time DESC,u.level desc,ee.add_time DESC';
...
//获取用户下面所有的产品
    ...
    public function getStaticProductsForpaimaiquan(int $limit, int $user_id = 0, int $type = 1, int $self_user_id = 0, int $goods_id, string $nickname, string $mobile, int $page = 1)
    {
        ...
        if ($goods_id > 0) {
            $order = '';
            //显示用户官网数据
            $hot_goods = Db::query("SELECT u.user_id,u.nickname,u.user_name,u.head_pic,u.is_authentication,u.nickname,u.level,u.fictitious,u.reg_time,g.* FROM tp_users u left JOIN `tp_goods` `g` ON `g`.`user_id`=`u`.`user_id`  {$where} {$order}  {$limit}");
        } else {
