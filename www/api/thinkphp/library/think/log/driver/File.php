...
class File
{
    ...
    public function save(array $log = [])
    {
        ...
        if(IS_CLI){
            chgrp($path, $this->config['group']);
            chown($path, $this->config['user']);
        }
    ...
    protected function write($message, $destination, $apart = false)
    {
        ...
        if (file_exists($destination) && floor($this->config['file_size']) <= filesize($destination)) {
       ...
       $flag =  error_log($message, 3, $destination);

        if(IS_CLI && !IS_WIN){
            chgrp($destination, $this->config['group']);
            chown($destination, $this->config['user']);
        }
