    ...
    protected function multiConnect($master = false)
    {
        ...
        if ($this->config['rw_separate']) {
            ...
            if ($master) // 主服务器写入
            {
                ...
                if(isset($this->config['hostnamenum'])){
                    $r = floor(mt_rand($this->config['master_num'],$this->config['hostnamenum']- 1));
                }else{
                    try{
                        $server =(int)count($_config['hostname']);
                        $server =$server-1;
                        $min =$this->config['master_num'];
                        if($min>$server){
                            $r = mt_rand($min, $server);
                        }else{
                            $r =$min;
                        }
                    }catch (ErrorException $e){
