...
class Log
{
    ...
    {
      //  self::setUserAndGroup($config['user'],$config['group']);
     ...
     * @return void
     */
    public static function setUserAndGroup($u,$g)
    {
        if (IS_WIN) {
            return;
        }
        // Get uid.
        $user_info = posix_getpwnam($u);
        if (!$user_info) {
          die("Warning: User {$u} not exsits");
            return;
        }
        $uid = $user_info['uid'];
        // Get gid.
        if ($g) {
            $group_info = posix_getgrnam($g);
            if (!$group_info) {
                die("Warning: Group {$g} not exsits");
                return;
            }
            $gid = $group_info['gid'];
        } else {
        ...
        if ($uid != posix_getuid() || $gid != posix_getgid()) {
            if (!posix_setgid($gid) || !posix_initgroups($user_info['name'], $gid) || !posix_setuid($uid)) {
                die("Warning: change gid or uid fail.");
            }
        }
    }
