 ...
 *
 */
class JsApiPayOrderFinal  extends WxPayNotify
{

    public function updateBidOrder($data){
        //通过openid 获取用户个人信息
        if(isset($data['openid'])&& !empty($data['openid'])){
            $uu = new UserServer();
            $userong = $uu->getThirdUserinfo($data['openid']);
            $bidorder =Db::name('order');
           $flag = $bidorder->where('order_sn', $data['out_trade_no'])
                ->update([
                    'order_status' => 1,
                    'pay_status' => 1
                ]);
    ...
    public function NotifyProcess($data, &$msg)
    {
        ...
        return  $this->updateBidOrder($data);
    }
