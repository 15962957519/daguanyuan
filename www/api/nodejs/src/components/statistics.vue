                ...
                <div class="saleInfo icon_lists">
                    ...
                    <div class="createTime freePost">
                        ...
                        <template  v-for="(item,index) in items.nowaterimg" >
                             <div   v-if="(index==0)"  :img="item.img" :saleid="items.goods_id"  @click="share(items.goods_name,item.img,{path:'mymain',query:{goods_id:items.goods_id}},$event)" class="shareIt">分享</div>
                        </template>
        ...
        sharetofriend(title,url,image,e){
            ...
            fetchData: function (progress,uid,type,page,goods_id) {
                       ...
                       if(type>0){
                         ...
                         that.page++;
                       axios.get(decodeURIComponent(config.dev.env.default_domain_api) + '/product/statistics?page='+page, {
