            ...
            <template v-if="$route.query.good_id >0">
                <div class="saleMain">
                    ...
                    <div class="goodInfo border horizonBottom">
                        <div class="img" v-bind:style="{backgroundImage:'url('+prouuctdata.image_url_remote_nowater+')'}">
                            <div class="freePost">
                                <i class="tianbaoweipai icon-freepost"></i>            </div>
                        </div>
                        <div class="desc">{{prouuctdata.goods_content}}</div>
                        <div class="endTime"><span>支付订单价格：</span><span>{{prouuctdata.bid_price}}</span></div>
                    </div>
                    <div class="saleArgs">
                        <div class="argItem">
                            <span>起拍价：</span>
        ...
        mounted: function() {
            ...
            geteditproductinfo(){
              Indicator.open({
                    ...
                    var order_id=    this.$route.query.order_id||0;
                    var order_sn=    this.$route.query.order_sn||'';
                    var is_finalpayment=   1;
                    var	that =this;
                    var dd =new FormData();
                    dd.append('token',storeWithExpiration.get('token'))
                    dd.append('good_id',good_id)
                    dd.append('order_id',order_id)
                    dd.append('order_sn',order_sn)
                    dd.append('is_finalpayment',is_finalpayment)
                    axios.post(decodeURIComponent(config.dev.env.default_domain_api)+'/index/getproductbyid', dd)
