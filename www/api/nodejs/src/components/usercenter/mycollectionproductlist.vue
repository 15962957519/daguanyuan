<template>
  <div  id="contentbox" class="contentbox" data-flag="mypublish">
                             ...
                             <div class="listContainer publish">
                                                 <div class="saleBox sale">
                                                     <div class="saleItem" :id="['item'+item.goods_id]" v-for="(item, index)  in lists">
                                                             ...
                                                             <div class="info">
                                                                <div class="content">
                                                                    <div class="goodsImg" v-bind:style="{backgroundImage:'url('+item.img+')'}"></div>
        ...
        <div class="my">
                <router-link class="tab-item" to="/usersellindex/productlist">我的发布</router-link>
                <a  class="tab-item selected" href="javascript:;">我的收藏</a>
           </div>
    ...
    methods:{
        getprouuct(){
          ...
          //  axios.defaults.headers.common['Authorization'] = 'Bearer'+storeWithExpiration.get('token');
               ...
               /*
                fetch(decodeURIComponent(config.dev.env.default_domain_api)+"/usercenterproduct/index",
                            {
                                method:'POST',
                                mode:'cors',
                                headers: {
...
return false;
                        //  fetch(decodeURIComponent(config.dev.env.default_domain_api)+"/usercenterproduct/del",
                          fetch(decodeURIComponent('http://localhost:999')+"/usercenterproduct/del",
                                    {
                                        method:'POST',
                                        mode:'cors',
                                        headers: {
